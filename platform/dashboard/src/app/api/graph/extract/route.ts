...
export async function POST(req: NextRequest) {
  try {
    ...
    // always dry_run from dashboard — preview only, no graph writes
    const payload = { document_id: 'preview', ...body, dry_run: true }
    const res = await fetch(`${GRAPH_URL}/graph/extract`, {
      method: 'POST',
